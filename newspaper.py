...
cursor.execute("create view article_views as select articles.title, articles.author, count(articles.slug) as views\
...
cursor.execute('select authors.name, sum(article_views.views) as views \
               from article_views join authors on article_views.author = authors.id \
               group by authors.name \
               order by views desc')
print "\nHere are the authors ranked in terms of greatest number of viewers:"
printRows(cursor.fetchall(), "views")
cursor.execute("select time::date as date, \
               round( count ( case when status like '%404%' then 1 end ) * 100::numeric / count(*) , 2 ) as percent_fail \
               from log \
               group by date \
               having (count(case when status like '%404%' then 1 end) * 100)::numeric / count(*) > 1.0;")
